...
export class SponsorController implements SponsorApiContract {
  ...
  async findOne(@Param('sponsorId') sponsorId: string): Promise<any> {
    try {
      ...
      Logger.log(
        'sponsor : ' + JSON.stringify(sponsorId),
        'sponsor.controller',
      );
      ...
      // result.imageUrl = fileSearchResult.
  ...
  async create(
    ...
    const res = await this.sponsorService.create(params);
    return res;
  }
  ...
  async uploadSponsorImage(
    ...
    if (!file) {
      throw new HttpException(
        { code: 'image_not_exist', message: 'Image is required' },
        400,
      );
    }

    Logger.log(
      'file added: ' + JSON.stringify(sponsorId),
      'sponsor.controller',
    );
    ...
    Logger.log('file uploaded: ' + JSON.stringify(file), 'sponsor.controller');
