...
import {
  ...
  UpdateImageResponse,
  UpdateImageRequest,
  BannersUpdateQueue, } from './banner.contract';
...
export class BannerController implements BannerApiContract {
  ...
  async create(
    ...
    try {
      ...
      const createdByUserId =
        req.user && req.user.userId ? req.user.userId : '';
  ...
  @Delete(':bannerId')
  // @UseGuards(AuthPermissionGuardV2('admin'))
  async delete(
    @Param('bannerId') bannerId: string,
  ...
  // @ApiBearerAuth()
  @Patch('queue/bulk')
  // @UseGuards(AuthPermissionGuardV2(getAllAdminRoles(ERoles)))
